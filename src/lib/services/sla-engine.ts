...
export function calculateSlaDeadline(category: string, severity: string, createdAt: Date): string {
...
export function getSlaRemaining(slaDeadline: string | Date): {
  hours: number;
  minutes: number;
  isBreached: boolean;
  percentage: number;
...
export function checkAndEscalate(incident: Incident): void {
  ...
  if (isBreached && !incident.slaBreached) {
    updateIncident(incident.id, { slaBreached: true });
    ...
    if (incident.userId) {
      createNotification({
        ...
        type: 'sla_breached',
        title: 'SLA Breached',
        message: `Incident ${incident.incidentId} has breached its SLA deadline.`,
      });
    }
  }
}
